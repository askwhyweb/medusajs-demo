...
export const getCacheOptions = async (
  ...
  // Always tag with the bare `tag` in addition to the session-scoped `tag-cacheId`.
  // Admin-triggered cache refreshes can't know a visitor's per-session cache id, so
  // revalidating the bare tag is what reliably busts cached data across all sessions
  // (including first-time visitors that have no cache id cookie yet).
...
export const getCountryCode = async () => {
  const cookies = await nextCookies()
  return cookies.get("_medusa_country_code")?.value?.toLowerCase()
}

export const setCountryCode = async (countryCode: string) => {
  const cookies = await nextCookies()
  cookies.set("_medusa_country_code", countryCode.toLowerCase(), {
    maxAge: 60 * 60 * 24 * 7,
    httpOnly: true,
    sameSite: "strict",
    secure: process.env.NODE_ENV === "production",
