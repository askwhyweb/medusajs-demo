...
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  ...
  return res.json({
    cache: cacheSettings.map(toSettingResponse),
    cms: cmsSettings.map(toSettingResponse),
    payment: paymentSettings[0] ? toSettingResponse(paymentSettings[0]) : null,
    shipping: shippingSettings.map(toSettingResponse),
  })
}
